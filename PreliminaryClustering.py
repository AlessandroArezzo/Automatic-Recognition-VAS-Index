...
class PreliminaryClustering:
    ...
    def __get_velocities_frames(self, coord_df_path, seq_df_path, num_lndks, selected_lndks_idx):
        ...
        for seq_num in np.arange(seq_df.shape[0]):
            ...
            lndks = lndks[:,2:]
            nose_tip_x = lndks[:,30]
            nose_tip_y = lndks[:,30+num_lndks]
            offset = np.hstack( (np.repeat(nose_tip_x.reshape(-1,1), num_lndks, axis=1),
                                 np.repeat(nose_tip_y.reshape(-1,1), num_lndks, axis=1) ) )
            ...
            lndk_vel = np.power(np.power(lndks_centered[0:lndks_centered.shape[0]-1,0:num_lndks] -
                                ...
                                np.power(lndks_centered[0:lndks_centered.shape[0]-1,num_lndks:] -
                                         lndks_centered[1:lndks_centered.shape[0],num_lndks:], 2),
                                0.5)
            data_velocities=[]
            for k in np.arange(lndk_vel.shape[0]):
                data_velocities.append(np.array(lndk_vel[k, selected_lndks_idx]).reshape(1, -1))
    ...
    def __generate_gmm(self, videos_features, n_kernels):
        print("---- Generate GMM with " + str(n_kernels) + " kernels... ----")
    ...
    def __calculate_FV(self, velocities, num_test_videos):
        print("---- Calculate fisher vectors of video sequences in dataset... ----")
        fisher_vectors=[]
        n_features_for_frame = velocities[0].shape[2]
        for i in range(0, num_test_videos):
    ...
    def __generate_histograms(self, n_kernels):
        ...
        for index in range(0, n_videos):
            ...
            video_histogram = np.zeros(n_kernels)
            for i in range(0, current_video.shape[0]):
                video_histogram=self.__clustering(current_video[i],video_histogram, n_kernels)
    ...
    the vas index """
    def __generate_relevant_and_neutral_configurations(self, histograms_of_videos, seq_df_path, threshold_neutral,
                                                     threshold_relevant):
    ...
    def execute_preliminary_clustering(self, coord_df_path, seq_df_path, num_lndks, selected_lndks_idx, num_test_videos,
                                       n_kernels, threshold_neutral, threshold_relevant,
                                       preliminary_clustering_dump_path=None, histo_figures_path=None,
                                       plot_and_save_histo=False):
        velocities = self.__get_velocities_frames(coord_df_path, seq_df_path, num_lndks, selected_lndks_idx)
        data_video_to_fit=self.__get_videos_frames_features(velocities)
        self.gmm = self.__generate_gmm(data_video_to_fit, n_kernels)
        self.fisher_vectors = self.__calculate_FV(velocities, num_test_videos)
        histograms_of_videos = self.__generate_histograms(n_kernels)
        self.index_relevant_configurations, self.index_neutral_configurations = self.__generate_relevant_and_neutral_configurations(
                                                histograms_of_videos, seq_df_path, threshold_neutral, threshold_relevant)
