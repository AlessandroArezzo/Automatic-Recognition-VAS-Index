...
class ModelClassifier:
    ...
    def __init__(self, type_classifier='SVM', regularization_parameter=1.0, gamma_parameter='scale'):
        self.type_classifier = type_classifier # Classifier to use: "SVM" or "SVR"
        self.regularization_parameter = regularization_parameter # Parameters to use for classifier
    ...
    def __get_multivector_sequences_descriptor(self, means_gmm_preliminary_clustering, relevant_configurations,
                                             num_test_videos, fisher_vectors):
    ...
    def __get_vas_videos_sequences(self, clusters_videos, seq_df_path):
    ...
    def __train_classifier(self, percent_training_set):
        ...
        if self.type_classifier == "SVM":
            classifier = svm.SVC(C=self.regularization_parameter, gamma=self.gamma_parameter)
        else:
            classifier = svm.SVR(C=self.regularization_parameter, gamma=self.gamma_parameter)
    ...
    def train_model(self, seq_df_path, num_test_videos, preliminary_clustering, percent_training_set,
                    classifier_dump_path=None):
        gmm_preliminary_clustering = preliminary_clustering.gmm
        relevant_configurations = preliminary_clustering.index_relevant_configurations
        fisher_vectors=preliminary_clustering.fisher_vectors
        clusters_videos = self.__get_multivector_sequences_descriptor(gmm_preliminary_clustering.means,
                                                                relevant_configurations, num_test_videos, fisher_vectors)
        ...
        self.vas_sequences=self.__get_vas_videos_sequences(clusters_videos, seq_df_path)
        self.classifier = self.__train_classifier(percent_training_set)
    ...
    def calculate_rate_model(self, path_scores_parameters=None):
        test_set_fv = self.__reshape_fv(np.array(self.fv_sequences))
        error = 0
        ...
        for num_video in np.arange(test_set_fv.shape[0]):
            real_vas = self.vas_sequences[num_video]
